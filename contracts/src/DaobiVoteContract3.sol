...
contract DaobiVoteContract is Initializable, ERC721Upgradeable, ERC721URIStorageUpgradeable, PausableUpgradeable, AccessControlUpgradeable, UUPSUpgradeable {
    ...
    uint256 public stake_amount; // 100 $DB with 18 decimals (default value)
    ...
    function register(address _initialVote, bytes32 _name) whenNotPaused public {
        ...
        // Check if already staked during mint, if not, require staking now
    ...
    function burn(address _account) public onlyRole(BURNER_ROLE) {
        ...
        // Burn stake by sending it to the DAO vault if it exists
        uint256 stakedAmount = stakes[_account];
        if (stakedAmount > 0) {
            delete stakes[_account];
            daobi.transfer(0, stakedAmount);
        }
    ...
    // Add a getter function for stake amount
    ...
    function setStakeAmount(_amount) public onlyRole(VOTE_ADMIN_ROLE) {
        stake_amount = _amount * 10**18;
    }
}